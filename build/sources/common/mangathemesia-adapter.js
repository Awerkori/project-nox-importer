...
export class MangaThemesiaAdapter {
    rateLimiter;
    transport;
    id;
    name;
    ...
    async fetchUpdatedWorks(cursor, options) {
        ...
        for (const m of matches) {
            ...
            if (seen.has(cleanSlug) || cleanSlug === 'feed' || cleanSlug === 'order')
                continue;
            seen.add(cleanSlug);
            works.push({
        ...
        if (readerareaMatch) {
            ...
            for (const m of imgs) {
                ...
                if (url.startsWith('data:')) {
                    ...
                    if (dsMatch && !dsMatch[1].startsWith('data:'))
                        url = dsMatch[1].trim();
                    else
                        continue;
                }
                if (/logo|banner|advert|discord/i.test(url))
                    continue;
                if (!seen.has(url)) {
                    seen.add(url);
                    pages.push(url);
