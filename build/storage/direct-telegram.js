...
export class BandwidthLimiter {
    ...
    tokens;
    lastRefill = Date.now();
    constructor(bytesPerSec = 4.0 * 1024 * 1024, maxBurst = 64 * 1024) {
...
export class DirectTelegramStorageProvider {
    logger = new Logger('DirectTelegramStorage');
    httpsAgent;
    bandwidthLimiter;
    bots = [];
    shards = [];
    metadataMap = new Map();
    lastBotRef = 'MANGA_STORAGE_01';
    lastShardId = null;
    lastChannelId = null;
    botRoundRobinIndex = 0;
    shardRoundRobinIndex = 0;
    ...
    loadConfiguration(checkpointPath) {
        let cp = null;
    ...
    setUploadRate(bytesPerSec) {
        this.bandwidthLimiter.setRate(bytesPerSec);
    }
    getProviderKey() {
        return 'telegram';
    }
    getLastBotReference(id) {
    ...
    selectOptimalShard(excludeIds = new Set()) {
        const now = Date.now();
        this.cleanRecentWindows(now);
            ...
            const req = https.request(`https://api.telegram.org/bot${token}/sendDocument`, {
                ...
                res.on('end', () => {
                    ...
                    try {
                        ...
                        const err = new Error(data.description || `Telegram upload failed HTTP ${res.statusCode}`);
                        err.statusCode = res.statusCode;
                        err.parameters = data.parameters;
                        reject(err);
                    }
                    catch (jsonErr) {
                        const err = new Error(`Invalid JSON response from Telegram: ${raw.slice(0, 100)}`);
                        err.statusCode = res.statusCode;
                        reject(err);
                    }
                });
        ...
        try {
            for (let attempt = 1; attempt <= maxAttempts; attempt++) {
                ...
                try {
                    ...
                    if (shard.latencies.length > 50)
                        shard.latencies.shift();
                    // Save metadata
                    ...
                    if (this.metadataMap.size > 2000) {
                        ...
                        if (firstKey)
                            this.metadataMap.delete(firstKey);
                    }
