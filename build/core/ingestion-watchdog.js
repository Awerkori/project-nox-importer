...
export class IngestionWatchdog {
    supabase;
    rateLimiter;
    logger = new Logger('IngestionWatchdog');
    constructor(supabase, rateLimiter) {
     ...
     */
    async checkLiveState(upstreamHasNewChapters = false) {
        if (!this.supabase) {
        ...
        for (const row of activeRows || []) {
            ...
            if (isExpired) {
                ...
                processing++;
                if (row.locked_by)
                    activeWorkers.add(row.locked_by);
            }
        }
