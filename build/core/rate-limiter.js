...
export class GlobalStorageRateLimiter {
    ...
    recordTransientError() {
        ...
        if (this.recentTransientErrors.length >= 8) {
            ...
            this.capacity = this.currentRatePerMinute;
            this.consecutiveSuccessfulUploads = 0;
            this.logger.warn(`[Storage AIMD 502] Concentrated transient errors (${this.recentTransientErrors.length} in 30s). Adjusting rate to ${this.currentRatePerMinute} req/min without global blocking.`);
        }
        else {
            this.logger.debug(`Recorded isolated transient error (${this.recentTransientErrors.length}/8 in 30s). No rate adjustment applied.`);
        }
    }
