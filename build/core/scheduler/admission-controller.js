...
export class AdmissionController {
    ...
    async canAdmitNewWork() {
      ...
      `);
            ...
            const inFlightRes = await client.query(`SELECT COUNT(*) as cnt FROM importer_queue WHERE status = 'IMPORTING' AND task_type = 'IMPORT_CHAPTER'`);
        ...
        try {
           ...
           LIMIT $2`, [activeIds.length > 0 ? activeIds : ['00000000-0000-0000-0000-000000000000'], newWorkSlotsAvailable * 3]);
                ...
                for (const cand of candidatesRes.rows) {
                    ...
                    try {
                        await client.query(`UPDATE importer_work_mappings 
               ...
               WHERE work_id = $1::uuid AND sync_status = 'WAITING_ADMISSION'`, [newWork.workId]);
                    ...
                    try {
                        const promRes = await client.query(`WITH to_promote AS (
