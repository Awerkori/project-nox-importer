    ...
    async handleProbeError(label, url, err) {
      ...
      `);
        ...
        if (count >= 5) {
            if (hasInfraPressure) {
                this.consecutivePreSlaViolations.set(label, 0);
                await this.triggerProtectiveStop(`Pre-SLA Health Probe Failed on ${label.toUpperCase()} (${count} consecutive failures) with Correlated Importer Pressure: ${err?.message} (YSQL: ${totalConns}/13 total [${activeConns} active], RSS: ${mem.rssMb}MB, Lag: ${lagMetrics.avgLagMs}ms)`, { label, url, error: err?.message, totalConns, activeConns, rssMb: mem.rssMb, lagMs: lagMetrics.avgLagMs });
            }
            else {
                this.logger.warn(`[EDGE_PROBE_ERROR_WARNING] Health probe on ${label} (${url}) failed (${count} consecutive): ${err?.message}, but importer infra is healthy (YSQL: ${totalConns}/13 total [${activeConns} active], RSS: ${mem.rssMb}MB). Observing without tripping PROTECTIVE_STOP.`);
            }
        }
    }
}
