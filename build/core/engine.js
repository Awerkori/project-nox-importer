...
export class ImporterEngine {
     ...
     */
    async runRedundantJobCleanupLoop() {
    ...
    async runLivenessWatchdogLoop() {
        ...
        while (!this.stopSignal) {
            ...
            try {
                ...
                const pool = getYugabytePool();
                const staleRes = await pool.query(`SELECT COUNT(*) as stale_importing_cnt
           FROM importer_queue 
           WHERE status = 'IMPORTING'
             AND (
               ...
               OR (locked_at <= NOW() - INTERVAL '15 minutes' AND updated_at <= NOW() - INTERVAL '15 minutes')
                ...
                let lastNewWorkCreatedAt = null;
                let lastNewWorkAdmittedAt = null;
          ...
          `);
                ...
                const latestNewWorkActivityMs = Math.max(lastNewWorkCreatedAt?.getTime() || 0, lastNewWorkAdmittedAt?.getTime() || 0);
                const minutesSinceLastNewWork = latestNewWorkActivityMs > 0
                    ? Math.floor((now - latestNewWorkActivityMs) / 60_000)
                    : 999;
                let newWorkPipeline = 'NEW_WORK_PIPELINE_HEALTHY';
                ...
                try {
                    const hbPayload = JSON.stringify({
                        ...
                        classificationCursor: healthMetrics.classificationCursor,
                        classifiedThisCycle: healthMetrics.classifiedThisCycle,
                        classificationCoverageEstimate: healthMetrics.classificationCoverageEstimate,
                        oldestUnclassifiedAge: healthMetrics.oldestUnclassifiedAge,
    ...
    async getEligibleChapterSources() {
        ...
        for (const src of candidateSources) {
            ...
            if (!this.circuitBreaker.canExecute(src))
                continue;
        ...
        if (!cached || now - cached.cachedAt > 10_000) {
            ...
            if (src) {
                cached = {
                    ...
                    catalogDiscoveryEnabled: src.catalog_discovery_enabled === true || src.catalog_discovery_enabled === 1,
    ...
    async runSourceWorker(source) {
        ...
        const slotPromises = Array.from({ length: concurrencySlots }, (_, slotIndex) => this.runSourceSlot(source, slotIndex));
        await Promise.all(slotPromises);
    }
    async runSourceSlot(source, slotIndex) {
    ...
    async runGeneralSlot(slotIndex) {
        ...
        while (!this.stopSignal) {
            try {
                ...
                const mutexWaitMs = this.chapterClaimMutex.getLastWaitMs?.() || 0;
            ...
            if (now - lastSync >= intervalMs) {
                ...
                try {
                    ...
                    let { data: existingActive } = typeof q.in === 'function'
                        ? await q.in('status', ['QUEUED', 'IMPORTING', 'RETRY']).limit(10)
                        : await q.limit(10);
        ...
        try {
            ...
            if (sourceRec) {
                ...
                if (sourceRec.status === 'PAUSED' || sourceRec.status === 'DISABLED' || !sourceRec.enabled) {
                    ...
                    await this.queue.releaseJob(job.id, 'PAUSED_BY_STAFF', `Source ${job.source} is ${sourceRec.status} (automatically parked by engine)`, 0, 'SOURCE_PAUSED');
                    return;
                }
                if (sourceRec.status === 'COOLDOWN') {
                    ...
                    if (Date.now() < cooldownUntil) {
                        ...
                        await this.queue.releaseJob(job.id, 'RETRY', `Source in COOLDOWN until ${sourceRec.cooldown_until}`, waitSeconds, 'SOURCE_TIMEOUT');
                        return;
                    }
                    else {
                        await this.supabase
            ...
            const statusFromErr = err?.status || (errorMessage.includes('403') ? 403 : errorMessage.includes('429') ? 429 : 500);
            const cfInsp = CloudflareClassifier.inspect(statusFromErr, err?.headers || {}, errorMessage, {
                expectedType: job.task_type === 'IMPORT_CHAPTER' ? 'image' : 'json',
                isIsolatedRequest: false,
            });
            const isUpstreamBlocked = ((cfInsp.isBlocked && statusFromErr !== 429) ||
                cfInsp.isChallenge ||
            ...
            this.logger.warn(`Job ${job.id} retry decision: ${decision.status} (delay: ${decision.delaySeconds}s, code: ${classification.taxonomyCode})`, {
                jobId: job.id,
                source: job.source,
            ...
            await this.queue.releaseJob(job.id, decision.status, this.sanitizeErrorMessage(structuredMsg), decision.delaySeconds, classification.taxonomyCode);
            if (job.payload?.workId) {
    ...
    async handleDiscoverWorks(job) {
        ...
        const isCompleted = Boolean(checkpoint?.metadata?.catalog_completed);
        const mode = job.payload?.mode || (isCompleted ? 'maintenance' : 'bootstrap');
    ...
    async handleSyncWork(job) {
        ...
        if (!coverMediaId && (job.payload?.workId || details.sourceWorkId)) {
            coverMediaId = await this.trySiblingMappingCoverFallback(job.payload?.workId, job.source, details.slug || details.title, botUserId);
        }
        const candidate = {
            source: job.source,
    ...
    async downloadAndRegisterImage(url, userId, purpose = 'editorial', source = 'unknown') {
    ...
    async fetchImageBytes(url, source = 'unknown', options) {
        const parsedUrl = new URL(url);
        ...
        if (!imgInsp.isValidImage || imgInsp.isBlocked || imgInsp.isChallenge || uint8.byteLength === 0) {
            ...
            if (imgInsp.isChallenge || imgInsp.isFakeContent) {
                throw new InvalidMediaError(url, source, `Cloudflare challenge HTML received instead of image (${cls}): ${imgInsp.reason}`);
            }
            if (!imgInsp.isValidImage) {
                throw new InvalidMediaError(url, source, `Invalid image format: magic bytes do not match PNG, JPEG, WebP, GIF or AVIF`);
            }
            throw new ProviderDownloadError(res.status === 200 ? 403 : res.status, url, source, `Cloudflare blocked image download (${cls}): ${imgInsp.reason}`);
        }
        this.rateLimiter.recordSuccess(parsedUrl.host);
        this.circuitBreaker.recordSuccess(source, parsedUrl.host);
        return uint8;
    }
    async tryRawMetadataCoverFallback(raw, botUserId, source, excludeUrl) {
        const candidateUrls = [
    ...
    async trySiblingMappingCoverFallback(workId, excludeSource, slugOrTitle, botUserId) {
        try {
                ...
                try {
                    ...
                    if (siblingAdapter && typeof siblingAdapter.fetchWorkDetails === 'function') {
                        ...
                        if (sibDetails?.coverUrl) {
                            const mediaId = await this.downloadAndRegisterImage(sibDetails.coverUrl, botUserId, 'editorial', m.source);
    ...
    async ensureWorkHasCover(workId, botUserId) {
        ...
        try {
            ...
            if (currentWork?.title) {
                ...
                if (siblingWorks && siblingWorks.length > 0) {
                    const siblingIds = siblingWorks.map((w) => w.id);
