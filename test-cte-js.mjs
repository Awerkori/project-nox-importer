...
async function run() {
  ...
  const { data: qJobs } = await supabase.from('importer_queue').select('id, chapter_sort_key, payload, status').in('status', ['QUEUED', 'RETRY']).lte('next_run_at', new Date().toISOString());
  
  const results = [];
  for(const q of qJobs) {
