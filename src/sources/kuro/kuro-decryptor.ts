...
function uless(a: number, b: number): boolean {
  return (a >>> 0) < (b >>> 0);
}
...
export class RabbitCipher {
  private x = new Uint32Array(8);
  private c = new Uint32Array(8);
  private b = 0;

  setup(key: Buffer, iv?: Buffer): void {
    if (key.length < 16) throw new Error('Key must be at least 16 bytes');
...
export function evpBytesToKey(
  ...
  while (derivedPos < derived.length) {
    ...
    if (md5Hash.length > 0) md.update(md5Hash);
    md.update(password);
    md.update(salt);
    md5Hash = md.digest();
...
export function derivePassword(dateStr?: string, encKey: string = DEFAULT_ENC_KEY): string {
...
let _cachedLiveKey: string | null = null;
let _lastKeyFetch = 0;

export async function fetchLiveEncryptionKey(baseUrl = 'https://kuromangas.com'): Promise<string> {
  const now = Date.now();
