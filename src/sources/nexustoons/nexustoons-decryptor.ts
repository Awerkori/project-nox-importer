...
let keys: KeyData[] | null = null;

function initializeKeys(): KeyData[] {
  if (keys) return keys;
...
function rotateRight(byte: number, shift: number): number {
  const s = shift % 8;
  return ((byte >>> s) | (byte << (8 - s))) & 0xFF;
}

export function isEncryptedNexusToons(data: any): boolean {
  return (
    data !== null &&
    typeof data === 'object' &&
...
export function decryptNexusToonsPayload<T = any>(data: { d: string; k?: number; v: number }): T {
