...
export class InkapkAdapter implements SourceAdapter {
  readonly id = 'inkapk';
  ...
  private async fetchHtml(url: string, options: RequestInit = {}): Promise<string> {
    ...
    while (attempts < maxAttempts) {
      ...
      try {
        ...
        if (!res.ok) {
          if (res.status === 403) {
            throw new Error(`Inkapk bloqueado por Cloudflare (HTTP 403): Just a moment... / Datacenter WAF`);
          }
          throw new Error(`Inkapk request failed: HTTP ${res.status}`);
        }
  ...
  async fetchUpdatedWorks(
    ...
    const url = page === 1 ? `${this.baseUrl}/obras/?sort=date` : `${this.baseUrl}/obras/page/${page}/?sort=date`;
    ...
    const hasNext = html.includes(`/page/${page + 1}/`) || html.includes('next page-numbers');
  ...
  async fetchWorkDetails(sourceWorkId: string): Promise<SourceWorkDetails> {
    const slug = sourceWorkId.replace(/^\//, '').replace(/\/$/, '').replace(/^obras\//, '');
    const url = `${this.baseUrl}/obras/${slug}/`;
    const html = await this.fetchHtml(url);

    const titleMatch = html.match(/class="[^"]*ink-det-title[^"]*"[^>]*>([\s\S]*?)<\//i) ||
                       html.match(/<h1[^>]*>([\s\S]*?)<\/h1>/i);
    const title = titleMatch ? decodeHtmlEntities(stripHtml(titleMatch[1])) : slug.replace(/[-_]+/g, ' ');

    const coverMatch = html.match(/class="[^"]*ink-det-cover[^"]*"[\s\S]*?<img[^>]+(?:src|data-src)="([^"]+)"/i) ||
                       html.match(/<div class="summary_image"[^>]*>[\s\S]*?<img[^>]+(?:src|data-src)="([^"]+)"/i);
    const coverUrl = coverMatch ? coverMatch[1].trim() : null;

    const descMatch = html.match(/class="[^"]*ink-det-desc[^"]*"[^>]*>([\s\S]*?)<\/div>/i) ||
    ...
    const artistMatch = html.match(/Arte[\s\S]*?<span>([\s\S]*?)<\/span>/i);
    const artist = artistMatch ? decodeHtmlEntities(stripHtml(artistMatch[1])) : undefined;

    const genres: string[] = [];
    const genreMatches = [
      ...html.matchAll(/class="[^"]*ink-genre-pill[^"]*"[^>]*>([\s\S]*?)<\//gi),
      ...html.matchAll(/class="genres-content"[\s\S]*?<a[^>]*>([\s\S]*?)<\/a>/gi),
    ];
  ...
  async fetchChapters(sourceWorkId: string): Promise<SourceChapterSummary[]> {
    ...
    let chapterBlocks = [
      ...html.matchAll(/<div class="[^"]*ink-ch-item[^"]*"[\s\S]*?<\/div>\s*<\/div>/gi),
      ...html.matchAll(/<li class="[^"]*wp-manga-chapter[^"]*"[\s\S]*?<\/li>/gi),
    ].map((m) => m[0]);

    if (chapterBlocks.length === 0) {
      try {
        const ajaxHtml = await this.fetchHtml(`${this.baseUrl}/obras/${slug}/ajax/chapters/`, {
          method: 'POST',
        });
    ...
    for (const block of chapterBlocks) {
      const linkMatch = block.match(/href="([^"]*\/obras\/[^"/]+\/([^"/]+)\/?)"/i);
      ...
      const titleMatch = block.match(/class="[^"]*ink-ch-item-name[^"]*"[^>]*>([\s\S]*?)<\//i) ||
  ...
  async fetchChapterPages(sourceChapterId: string, _chapterNumber?: number): Promise<string[]> {
    ...
    const url = `${this.baseUrl}/obras/${chPath}/?style=list`;
    const html = await this.fetchHtml(url);
