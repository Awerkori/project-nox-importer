...
export class WpGalleryAdapter implements SourceAdapter {
  readonly id: string;
  ...
  async fetchUpdatedWorks(
    ...
    try {
      const posts = await this.fetchJson<any[]>(url);
  ...
  async fetchWorkDetails(sourceWorkId: string): Promise<SourceWorkDetails> {
    ...
    try {
      const p = await this.fetchJson<any>(url);
      const title = decodeHtmlEntities(stripHtml(p.title?.rendered || '')).trim();
  ...
  async fetchChapterPages(sourceChapterId: string): Promise<string[]> {
    ...
    try {
      ...
      const p = await this.fetchJson<any>(postUrl);
      ...
      if (pages.length === 0 && link) {
        ...
        const gallerySubMatch =
          html.match(/href=["']([^"']+)["'][^>]*title=["']Abrir galeria["']/i) ||
        ...
        if (pages.length === 0) {
          ...
          const galleryImgs = html.matchAll(
            /(?:class=["'][^"']*(?:post-fotos|listaImagens|galeria|galeria-foto|entry-content)[^"']*["'][\s\S]*?)?<img[^>]+(?:data-src|src)=["']([^"']+)["']/gi
          );
