  ...
  async fetchChapterPages(sourceChapterId: string, _chapterNumber?: number): Promise<string[]> {
    ...
    for (const tag of imgTags) {
      ...
      if (/dflazy|placeholder|loading/i.test(candidate)) {
        if (dataSrc && !/dflazy|placeholder/i.test(dataSrc)) candidate = dataSrc.trim();
        else if (dataFull && !/dflazy|placeholder/i.test(dataFull)) candidate = dataFull.trim();
        else continue;
      }

      const filename = candidate.split('/').pop()?.split('?')[0] || '';
      if (/(?:^|[_\-.])(logo|avatar|icon|banner|ads|advert|discord|telegram)(?:[_\-.]|$)/i.test(filename)) continue;
