...
export class HanamiHeavenAdapter implements SourceAdapter {
  readonly id = 'hanamiheaven';
  readonly name = 'Hanami Heaven';
  readonly baseUrl = 'https://hanamiheaven.org';

  private logger = new Logger('HanamiHeavenAdapter');
  ...
  private async solveJsChallenge(): Promise<boolean> {
    try {
    ...
    while (attempts < maxAttempts) {
      ...
      try {
        ...
        if (res.status === 403) {
          ...
          if (bypassed && attempts < maxAttempts) continue;
        }
