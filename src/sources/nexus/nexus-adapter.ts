...
const ANON_KEY =
  'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJzdXBhYmFzZSIsImlhdCI6MTc4NzgwMjAwMCwiZXhwIjo0OTQzNDc1NjAwLCJyb2xlIjoiYW5vbiJ9.Cnl8Jw2DeKe84OAkmJYfO33xlcZsw0TC2Nw_il0tpRs';
...
export class NexusAdapter implements SourceAdapter {
  ...
  private apiUrl = 'https://supabase.nexusmangas.com/rest/v1';
  private functionsUrl = 'https://supabase.nexusmangas.com/functions/v1';
  private logger = new Logger('NexusAdapter');
  ...
  async fetchUpdatedWorks(
    ...
    const workRows = await this.request<
      Array<{ id: string; title: string; slug: string; cover_url?: string; updated_at?: string }>
    >(worksUrl);

    const summaries: SourceWorkSummary[] = (workRows || []).map((w) => ({
      sourceWorkId: w.id,
      title: w.title,
      slug: w.slug,
  ...
  async fetchWorkDetails(sourceWorkId: string): Promise<SourceWorkDetails> {
    ...
    const rows = await this.request<any[]>(url);
    const row = rows?.[0];
    ...
    if (Array.isArray(row.work_genres)) {
      for (const wg of row.work_genres) {
        if (wg.genre?.name) genres.push(wg.genre.name);
      }
    }
