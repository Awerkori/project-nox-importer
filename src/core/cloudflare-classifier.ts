export type CloudflareClassification =
  | 'HOTLINK_REFERER_REQUIRED'
  | 'RATE_LIMITED'
  | 'BURST_LIMITED'
  | 'TEMPORARY_WAF'
  | 'DATACENTER_ASN_BLOCK'
  | 'CLOUDFLARE_DATACENTER_BLOCK'
  | 'JS_CHALLENGE'
  | 'TURNSTILE'
  | 'IMAGE_CDN_BLOCK'
  | 'API_BLOCK'
...
export class CloudflareClassifier {
   ...
   */
  private static isBinaryImage(buffer: ArrayBuffer | Uint8Array): boolean {
  ...
  public static inspect(
    ...
    const hasChallengeIndicators =
      combinedBody.includes('just a moment...') ||
    ...
    let classification: CloudflareClassification | null = null;
    let reason = 'OK';
