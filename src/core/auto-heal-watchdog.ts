  ...
  async executeRecoveryLadder(metrics: HealthPanelMetrics): Promise<void> {
    ...
    if (progressAgeSec >= 30 * 60 && metrics.status === 'CRITICAL_STALL') {
      ...
      if (this.circuitBreakerOpen || metrics.autoRestartCount1h >= 3) {
        ...
        this.logger.error(
          `🚨 [AUTO-RECOVERY CIRCUIT OPEN] Reached max 3 auto-restarts in 1h (Current count: ${metrics.autoRestartCount1h}). Halting automatic restarts to prevent loop. ROOT CAUSE REQUIRED.`
        );
        return;
      }
