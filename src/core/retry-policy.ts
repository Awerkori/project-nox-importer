...
export type RetryErrorClass =
  | 'LOCAL_RETRY'
  | 'QUEUE_RETRY_STORAGE_502'
  | 'QUEUE_RETRY_STORAGE_503'
  | 'QUEUE_RETRY_TIMEOUT'
  | 'QUEUE_RETRY_429'
  | 'QUEUE_RETRY_PROVIDER'
  | 'QUEUE_RETRY_PERMANENT'
  | 'FAILED';
...
export const CATEGORY_RETRY_BUDGET: Record<ErrorTaxonomyCode, number> = {
...
export class RetryPolicy {
  ...
  static classify(err: any): RetryClassification {
    ...
    if (
      /failed to return pages|returned 0 pages|contains 0 valid content pages|0 valid content pages|incomplete chapter import/i.test(message)
    ) {
      return {
    ...
    if (
      ...
      if (status === 429 || /429|rate\s*limit|floodwait/i.test(message)) {
        let waitSec = 60;
        if (typeof (err as any).retryAfter === 'number' && (err as any).retryAfter > 0) {
          waitSec = (err as any).retryAfter;
        } else {
    ...
    if (status === 429 || /429|rate\s*limit|floodwait/i.test(message)) {
      ...
      if (retryAfter) {
        ...
        if (!isNaN(parsed) && parsed > 0) waitSec = Math.min(3600, parsed);
      }
  ...
  static decide(
    ...
    _options?: { isStaffPriority?: boolean }
  ): RetryDecision {
    ...
    const effectiveBudget = Math.min(
      Math.max(1, maxAttempts || MAX_RETRY_BUDGET_GLOBAL),
      categoryBudget,
      MAX_RETRY_BUDGET_GLOBAL
    );
