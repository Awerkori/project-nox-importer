...
export class WorkAffinityScheduler {
  ...
  private lastClaimTime = Date.now();
  private lastCompletionTime = Date.now();
  ...
  public specificClaimSuccesses = 0;
  public genericClaimAttempts = 0;
  public genericClaimSuccesses = 0;
  public emptyClaimAttempts = 0;

  // Lightweight probe for P0 queue presence (avoids 400ms scan on every claim)
  private lastP0ProbeAt = 0;
  private hasP0InQueue = false;

  private async hasP0Candidate(): Promise<boolean> {
    if (process.env.NODE_ENV === 'test' || !this.pool?.query) {
      return true;
    }
    const now = Date.now();
    if (this.hasP0InQueue) return true;
    if (now - this.lastP0ProbeAt < 5000) return false;
  ...
  private stagedBlockedWorks = new Map<string, number>();

  public markWorkStagedBlocked(workId: string, ttlMs: number = 15000): void {
    this.stagedBlockedWorks.set(workId, Date.now() + ttlMs);
  }

  public isWorkStagedBlocked(workId: string): boolean {
  ...
  public clearWorkStagedBlocked(workId: string): void {
    this.stagedBlockedWorks.delete(workId);
  }
  ...
  private async runQuery(clientOrPool: any, text: string, params?: any[]): Promise<any> {
   ...
   */
  public getTotalInFlight(): number {
    let total = 0;
   ...
   */
  public getFullInFlightWorkIds(maxInFlight: number = 2): string[] {
    const full: string[] = [];
  ...
  private async executeIntelligentClaim(
    ...
    const staffForcedJob = await this.claimStaffForcedJob(this.pool, {
      ...
      // Check critical gaps first (only for sources with available permits)
      const criticalWorks = p1Works.filter(
        (w) => w.criticalGapSortKey !== null &&
      ...
      const eligibleP1Works = p1Works.filter(
        (w) => w.state === 'FILLING' && w.criticalGapSortKey === null && !this.isWorkStagedBlocked(w.workId) && (this.inFlightByWork.get(w.workId) || 0) < config.maxInflightPerWork
      );
      ...
      const eligibleP2Works = p2Works.filter(
        (w) => !this.isWorkStagedBlocked(w.workId) && (this.inFlightByWork.get(w.workId) || 0) < config.maxInflightPerWork
      );
   ...
   */
  private async claimStaffForcedJob(
    client: any,
    opts: {
      workerId: string;
      leaseMin: number;
    ...
    // 1. Check for active staff requests first (very cheap indexed lookup ~1.5ms)
    ...
    const staffWorkIds = activeReqs.rows.map((r: any) => r.work_id);
   ...
   */
  public async validateClaimedJobPostMutex(job: any): Promise<{ valid: boolean; reason?: string }> {
      ...
      try {
        ...
        if (hasSafeWork) {
          ...
          if (elapsedPubMs >= 10 * 60 * 1000) {
            this.logger.error(
              `🚨 [PUBLICATION_WATCHDOG] Pipeline stalled: 0 publications for ${(elapsedPubMs / 60000).toFixed(1)}m while safe backlog exists. ` +
            ...
            this.logger.warn(
              `⚠️ [PUBLICATION_WATCHDOG] Warning: 0 publications for ${(elapsedPubMs / 60000).toFixed(1)}m. ` +
