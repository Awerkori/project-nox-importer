 ...
 */
export const KNOWN_CANONICAL_SYNONYM_GROUPS: string[][] = [
  [
    'imperador magico',
    'imperador demoniaco',
    'magic emperor',
 ...
 */
export function getCanonicalSynonyms(text: string): string[] {
  if (!text) return [];
  const norm = normalizeTitle(text);
  if (!norm) return [];

  const matchedGroup = KNOWN_CANONICAL_SYNONYM_GROUPS.find((group) =>
    group.some((member) => {
 ...
 */
export function matchWorkCandidate(target: MatchCandidate, candidate: MatchCandidate): WorkMatchResult {
  ...
  if (
    targetSynonyms.includes(normCand) ||
  ...
  if (normTarget.length >= 3 && normCand.length >= 3) {
    ...
    const isSubstring =
      (normTarget.includes(normCand) && candWords.length >= 2) ||
