  ...
  private async runLivenessWatchdogLoop(): Promise<void> {
    ...
    while (!this.stopSignal) {
      ...
      try {
        ...
        let livenessState: 'HEALTHY_IDLE' | 'HEALTHY_WORKING' | 'BACKPRESSURED' | 'STALLED' = 'HEALTHY_WORKING';
        ...
        if (livenessState === 'STALLED') {
          if (minutesSinceProgress >= 30) {
            this.logger.error(
              `🚨 [LIVENESS CRITICAL] Importer STALLED: ${eligibleCount} eligible jobs waiting, but NO progress for ${minutesSinceProgress} minutes! Triggering aggressive auto-recovery...`
            );
          } else {
            this.logger.warn(
              `⚠️ [LIVENESS WARNING] Importer STALLED: ${eligibleCount} eligible jobs waiting, but NO progress for ${minutesSinceProgress} minutes. Triggering auto-recovery...`
            );
          }
