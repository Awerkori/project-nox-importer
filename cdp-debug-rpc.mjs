...
async function run() {
  ...
  const sql = `
    CREATE OR REPLACE FUNCTION public.debug_acquire_job() RETURNS jsonb AS $$
    DECLARE
      v_res jsonb;
    BEGIN
      ...
      recovery_candidates AS (
        SELECT id, task_type, source, effective_priority, payload, chapter_sort_key, created_at, next_run_at, current_inflight
