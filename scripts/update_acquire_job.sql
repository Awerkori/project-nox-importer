...
BEGIN
  ...
  -- 2. Select next job using Anti-Starvation Banding (UNION ALL) and Fairness penalty
  SELECT q.id INTO v_job_id
  FROM public.importer_queue q
  WHERE q.id = (
    SELECT cand_batch.id
    FROM (
      -- BAND 1: Fresh / Normal (priority >= 100)
      ...
      -- BAND 2: Gap / Recovery (priority 80 to 99)
      SELECT q_cand.id, q_cand.task_type, q_cand.priority, q_cand.payload, q_cand.chapter_sort_key, q_cand.created_at
      FROM public.importer_queue q_cand
      WHERE q_cand.status in ('QUEUED', 'RETRY')
        AND q_cand.priority >= 80 AND q_cand.priority < 100
        AND q_cand.next_run_at <= now()
        AND (v_focus_work_id is null or (q_cand.payload->>'workId')::text = v_focus_work_id::text)
        AND (p_source is null or q_cand.source = p_source)
        AND (p_task_type is null or (p_task_type = 'DISCOVERY' and q_cand.task_type in ('DISCOVER_WORKS', 'SYNC_WORK')) or q_cand.task_type = p_task_type)
      ...
      -- BAND 3: Historical / Low (priority < 80)
      SELECT q_cand.id, q_cand.task_type, q_cand.priority, q_cand.payload, q_cand.chapter_sort_key, q_cand.created_at
      FROM public.importer_queue q_cand
      WHERE q_cand.status in ('QUEUED', 'RETRY')
        AND q_cand.priority < 80
        AND q_cand.next_run_at <= now()
        AND (v_focus_work_id is null or (q_cand.payload->>'workId')::text = v_focus_work_id::text)
        AND (p_source is null or q_cand.source = p_source)
        AND (p_task_type is null or (p_task_type = 'DISCOVERY' and q_cand.task_type in ('DISCOVER_WORKS', 'SYNC_WORK')) or q_cand.task_type = p_task_type)
    ...
    ORDER BY
      -- Absolute Priority Focus Work ignores fairness penalties
      ...
      -- Fairness and Gap Evaluation
      (
        -- Base priority
        cand_batch.priority 
        
        -- Gap Upgrade: If there is a STAGED chapter ahead of us, we are a GAP FILLER. Upgrade us massively so we beat fresh chapters of OUR work!
        ...
        -- Fairness Penalty: -1000 per active IMPORTING job for this work. Prevents work monopoly.
      ...
      -- For ties (e.g. multiple chapters of the same work in the candidate pool), ALWAYS pick the lowest chapter_sort_key first!
  ...
  -- 2b. Fallback: Check expired leases
