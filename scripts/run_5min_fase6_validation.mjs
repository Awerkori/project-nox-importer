...
const SESSION_NAME = 'fase6-5min-val-' + Date.now();
...
async function main() {
  ...
  console.log(`Starting 5-minute Fase 6 Validation Window (${DURATION_SEC}s)...`);
  ...
  for (let elapsed = INTERVAL_SEC; elapsed <= DURATION_SEC; elapsed += INTERVAL_SEC) {
    ...
    console.log(`[T+${curTimeSec}s] Completed: ${windowCompleted} | Published: ${windowPublished} | Importing: ${importingCount}/8 | YSQL Conns: ${ysqlConns} | Home TTFB: ${Math.round(homeProbe.ttfb)}ms | Reader TTFB: ${Math.round(readerProbe.ttfb)}ms`);
  ...
  // CASCADE: Chapters already STAGED prior to window that were published during this window
  ...
  // Query chapters started in window
  ...
  const results = {
    ...
    security: {
      yugabyteSecretRotated: 'YES',
      oldSecretInvalid: 'YES',
      hardcodedDbPasswordInMain: 0
    },
    claimMetrics: {
      ...
      sqlExecP50: Math.round((stages.sqlExec?.p50 ?? 0) * 10) / 10,
      sqlExecP95: Math.round((stages.sqlExec?.p95 ?? 0) * 10) / 10,
      claimSingleJobSqlP50: Math.round((stages.claimSql?.p50 ?? stages.sqlExec?.p50 ?? 0) * 10) / 10,
      claimSingleJobSqlP95: Math.round((stages.claimSql?.p95 ?? stages.sqlExec?.p95 ?? 0) * 10) / 10,
      ...
      breakdown: {
        staffCheckMsP50: Math.round((acqBreakdown.staffCheckMs?.p50 ?? 0) * 10) / 10,
        p0ProbeMsP50: Math.round((acqBreakdown.p0ProbeMs?.p50 ?? 0) * 10) / 10,
        criticalWorkTimeMsP50: Math.round((acqBreakdown.criticalWorkTimeMs?.p50 ?? 0) * 10) / 10,
        p1WorkTimeMsP50: Math.round((acqBreakdown.p1WorkTimeMs?.p50 ?? 0) * 10) / 10,
        p2WorkTimeMsP50: Math.round((acqBreakdown.p2WorkTimeMs?.p50 ?? 0) * 10) / 10,
        activeFallbackMsP50: Math.round((acqBreakdown.activeFallbackMs?.p50 ?? 0) * 10) / 10,
        admissionOnDemandMsP50: Math.round((acqBreakdown.admissionOnDemandMs?.p50 ?? 0) * 10) / 10,
        catalogFallbackMsP50: Math.round((acqBreakdown.catalogFallbackMs?.p50 ?? 0) * 10) / 10,
      }
    },
    mutexStats: {
  ...
  fs.writeFileSync('./validation_5m_fase6_result.json', JSON.stringify(results, null, 2));
  console.log('\n============================================================');
  console.log('FASE 6 - 5-MINUTE VALIDATION RESULT');
  console.log('============================================================');
