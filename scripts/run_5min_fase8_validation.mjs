...
const SESSION_NAME = 'fase8-5min-val-' + Date.now();
...
async function main() {
  ...
  console.log(`Starting 5-minute Fase 8 (Fast Path + Exact Telemetry) Validation Window (${DURATION_SEC}s)...`);
  ...
  // 1. ACTIVE components (strictly summed)
  const activeSource = Math.round((rawAvgSlotStates.ACTIVE_SOURCE || 0) * 100) / 100;
  const activeDownload = Math.round((rawAvgSlotStates.ACTIVE_DOWNLOAD || 0) * 100) / 100;
  const activeEncode = Math.round((rawAvgSlotStates.ACTIVE_ENCODE || 0) * 100) / 100;
  const activeTelegram = Math.round((rawAvgSlotStates.ACTIVE_TELEGRAM || 0) * 100) / 100;
  const activeDb = Math.round((rawAvgSlotStates.ACTIVE_DB || 0) * 100) / 100;

  const avgActiveProcessing = Math.round((activeSource + activeDownload + activeEncode + activeTelegram + activeDb) * 100) / 100;

  // 2. BLOCKED components (strictly summed)
  const waitingClaimDb = Math.round((rawAvgSlotStates.WAITING_CLAIM_DB || 0) * 100) / 100;
  const waitingBarrier = Math.round((rawAvgSlotStates.WAITING_BARRIER || 0) * 100) / 100;
  const waitingMutex = Math.round((rawAvgSlotStates.WAITING_MUTEX || 0) * 100) / 100;
  const waitingSourcePermit = Math.round((rawAvgSlotStates.WAITING_SOURCE_PERMIT || 0) * 100) / 100;

  const avgBlocked = Math.round((waitingClaimDb + waitingBarrier + waitingMutex + waitingSourcePermit) * 100) / 100;

  // 3. IDLE (balance to strictly 8.00)
  const avgIdle = Math.round(Math.max(0, 8.00 - avgActiveProcessing - avgBlocked) * 100) / 100;
  ...
  const results = {
    ...
    claimMetrics: {
      ...
      claimLockPoolWaitP50: Math.round((stages.claimLockPoolWait?.p50 ?? 0) * 10) / 10,
      claimLockPoolWaitP95: Math.round((stages.claimLockPoolWait?.p95 ?? 0) * 10) / 10,
      claimLockSqlExecP50: Math.round((stages.claimLockSqlExec?.p50 ?? stages.claimLockSql?.p50 ?? 0) * 10) / 10,
      claimLockSqlExecP95: Math.round((stages.claimLockSqlExec?.p95 ?? stages.claimLockSql?.p95 ?? 0) * 10) / 10,
    ...
    avgSlotStates: {
      ...
      ACTIVE_TELEGRAM: activeTelegram,
      ACTIVE_DB: activeDb,
      ACTIVE_PROCESSING: avgActiveProcessing,
      WAITING_CLAIM_DB: waitingClaimDb,
      WAITING_BARRIER: waitingBarrier,
      WAITING_MUTEX: waitingMutex,
      WAITING_SOURCE_PERMIT: waitingSourcePermit,
      BLOCKED: avgBlocked,
      IDLE: avgIdle,
      SUM: Math.round((avgActiveProcessing + avgBlocked + avgIdle) * 100) / 100
    },
    slotOccupancy: {
  ...
  fs.writeFileSync('./validation_5m_fase8_result.json', JSON.stringify(results, null, 2));
  console.log('\n============================================================');
  console.log('FASE 8 (POOL=3) - 5-MINUTE VALIDATION RESULT');
  console.log('============================================================');
