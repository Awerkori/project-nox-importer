...
// Fix 2: ambiguous "attempts" reference in UPDATE and fallback UPDATE
// Solution: use a single atomic update using a subquery to get current values first
// OR use unambiguous column references via CTE approach
const sql = `
...
BEGIN
  ...
  IF v_focus_request_id IS NOT NULL THEN
    ...
    IF NOT v_has_pending_jobs AND NOT v_has_pending_mappings AND COALESCE(v_has_completed_work, false) THEN
      UPDATE public.importer_staff_requests sr2
      SET status = 'COMPLETED', updated_at = now()
      WHERE sr2.id = v_focus_request_id;
      v_focus_request_id := NULL;
      v_focus_work_id := NULL;
    ELSIF v_focus_status = 'QUEUED' THEN
      UPDATE public.importer_staff_requests sr2
      SET status = 'IMPORTING', updated_at = now()
      WHERE sr2.id = v_focus_request_id AND sr2.status = 'QUEUED';
    END IF;
  ...
  IF v_job_id IS NULL THEN
    ...
    WHERE q_stale.status = 'IMPORTING'
      AND q_stale.lease_expires_at < now()
      AND (q_stale.task_type IN ('DISCOVER_WORKS', 'SYNC_WORK') OR COALESCE(v_barrier_state, 'CLOSED') IN ('OPEN', 'CAUTION') OR EXISTS (
        SELECT 1 FROM public.importer_chapter_mappings m2 WHERE m2.status = 'STAGED'
        AND m2.work_id::text = q_stale.payload->>'workId' AND m2.chapter_sort_key > q_stale.chapter_sort_key
      ))
    ...
    ORDER BY q_stale.lease_expires_at ASC
    LIMIT 1
    FOR UPDATE SKIP LOCKED;

    IF v_job_id IS NOT NULL THEN
      -- Use explicit table alias to avoid ambiguous column references
      UPDATE public.importer_queue iq
      SET iq.attempts = iq.attempts + 1,
          iq.status = 'RETRY',
          iq.last_error = 'Lease reclaimed after expiry',
          iq.updated_at = now()
      WHERE iq.id = v_job_id;
    END IF;
  END IF;

  IF v_job_id IS NULL THEN RETURN; END IF;

  -- Check if this was QUEUED (to know whether to increment attempts)
  SELECT (q_chk.status = 'QUEUED') INTO v_was_queued
  FROM public.importer_queue q_chk
  WHERE q_chk.id = v_job_id;

  -- Final update: fully qualified to avoid any ambiguous column references
  UPDATE public.importer_queue iq2
  SET
    iq2.status = 'IMPORTING',
    iq2.locked_by = p_worker_id,
    iq2.locked_at = now(),
    iq2.lease_expires_at = now() + p_lease_duration,
    iq2.attempts = CASE WHEN v_was_queued THEN iq2.attempts + 1 ELSE iq2.attempts END,
    iq2.updated_at = now()
  WHERE iq2.id = v_job_id;
  ...
  FROM public.importer_queue q_ret
  WHERE q_ret.id = v_job_id;
END;
$body$;
...
async function run() {
  ...
  console.log("Hotfix2 result:", result.status, result.body.slice(0, 300));
