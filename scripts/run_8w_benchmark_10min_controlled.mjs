...
async function main() {
  ...
  console.log('======================================================================');
  console.log(`🚀 PROJECT NOX — CONTROLLED 8-WORKER PURE DIRECT BENCHMARK (${durationMinutes} MINUTES)`);
  ...
  try {
    while (Date.now() - startTime < durationMs) {
      ...
      const sample = {
        ...
        concurrency: parseInt(telem.concurrency || 8, 10),
        importingNow,
        queuedNow,
        completedChapters,
        failedChapters,
    ...
    console.log('\n======================================================================');
    console.log('📊 8-WORKER 10-MINUTE PURE DIRECT BENCHMARK OFFICIAL SUMMARY');
    console.log('======================================================================');
    console.log(JSON.stringify(summary, null, 2));

    fs.writeFileSync('benchmark_8w_10min_pure_result.json', JSON.stringify({ summary, samples }, null, 2));
    fs.writeFileSync('benchmark_8w_10min_result.json', JSON.stringify({ summary, samples }, null, 2));
    console.log('\n✅ Detailed results written to benchmark_8w_10min_pure_result.json');
