...
async function main() {
  ...
  console.log('======================================================================');
  console.log(`🚀 PROJECT NOX — CONTROLLED 5-WORKER BENCHMARK (${durationMinutes} MINUTES)`);
  console.log(`   Scope: IMPORT_CHAPTER only | Discovery: PAUSED | Reconciler: ACTIVE`);
  ...
  console.log('✅ Publication Safety Barrier is OPEN. Runners will start acquiring chapters.\n');
  ...
  try {
    while (Date.now() - startTime < durationMs) {
      ...
      console.log(
        ...
        `YSQL: ${sample.totalConns}/${maxConnections} (Hyp: ${sample.hyperdriveConns}, Act: ${sample.activeConns}) | ` +
        `Discloud: ${sample.discloudCpu}% CPU, ${sample.discloudRam}MB RAM, lag: ${sample.eventLoopLag}ms | ` +
    ...
    // Extract Telegram 429 and FloodWait from final logs
    const newLogChunk = finalLogs.replace(initialLogs, '');
    ...
    console.log('\n======================================================================');
    console.log('📊 5-WORKER 10-MINUTE PURE BENCHMARK OFFICIAL SUMMARY');
    console.log('======================================================================');
    console.log(JSON.stringify(summary, null, 2));

    fs.writeFileSync('benchmark_5w_10min_pure_result.json', JSON.stringify({ summary, samples }, null, 2));
    fs.writeFileSync('benchmark_5w_10min_result.json', JSON.stringify({ summary, samples }, null, 2));
    console.log('\n✅ Detailed results written to benchmark_5w_10min_pure_result.json');
