...
// PostgreSQL UPDATE: SET clause uses bare column names ONLY
// The ambiguity was ONLY in the CASE WHEN clause
// Fix: use v_was_queued variable to avoid any reference to table column in CASE
const sql = `
...
BEGIN
  ...
  IF v_focus_request_id IS NOT NULL THEN
    ...
    ELSIF v_focus_status = 'QUEUED' THEN
      ...
      WHERE id = v_focus_request_id AND status = 'QUEUED';
    END IF;
  ...
  -- Fallback: reclaim stale IMPORTING leases (no ambiguous columns here - subquery alias scoped)
  IF v_job_id IS NULL THEN
    ...
    WHERE q_stale.status = 'IMPORTING'
      ...
      AND (q_stale.task_type IN ('DISCOVER_WORKS', 'SYNC_WORK')
           OR COALESCE(v_barrier_state, 'CLOSED') IN ('OPEN', 'CAUTION')
           OR EXISTS (
             SELECT 1 FROM public.importer_chapter_mappings m2
             WHERE m2.status = 'STAGED'
               AND m2.work_id::text = q_stale.payload->>'workId'
               AND m2.chapter_sort_key > q_stale.chapter_sort_key
           ))
    ...
    IF v_job_id IS NOT NULL THEN
      -- Read current attempts before UPDATE to avoid ambiguity in SET
      SELECT q_pre.attempts + 1 INTO v_new_attempts
      FROM public.importer_queue q_pre WHERE q_pre.id = v_job_id;
  ...
  -- Read current status + attempts into variables BEFORE the UPDATE
  -- This avoids ANY ambiguous column reference in the SET clause
  SELECT
    (q_pre2.status = 'QUEUED'),
    CASE WHEN q_pre2.status = 'QUEUED' THEN q_pre2.attempts + 1 ELSE q_pre2.attempts END
  INTO v_was_queued, v_new_attempts
  FROM public.importer_queue q_pre2
  WHERE q_pre2.id = v_job_id;
...
async function run() {
  ...
  console.log("Final hotfix result:", result.status, result.body.slice(0, 300));
