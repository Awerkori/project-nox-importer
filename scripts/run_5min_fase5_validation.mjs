...
const SESSION_NAME = 'fase5-5min-val-' + Date.now();
...
async function main() {
  ...
  console.log(`Starting 5-minute Fase 5 Validation Window (${DURATION_SEC}s)...`);
  console.log(`Diagnostic Session: ${SESSION_NAME}`);

  // Activate diagnostic session in settings (clean string without extra quotes)
  ...
  for (let elapsed = INTERVAL_SEC; elapsed <= DURATION_SEC; elapsed += INTERVAL_SEC) {
    ...
    // 5. Audit Active Works for Zombies
    try {
      const actRes = await client.query("SELECT value FROM settings WHERE key = 'active_works'");
      const activeWorks = actRes.rows[0]?.value ? (typeof actRes.rows[0].value === 'string' ? JSON.parse(actRes.rows[0].value) : actRes.rows[0].value) : [];
      let zombies = 0;
      let claimable = 0;
  ...
  // FRESH NEW VISIBLE: Chapters newly completed/staged and published within this window
  ...
  const startedRes = await client.query(`
    SELECT count(*) as count 
    FROM importer_queue 
    WHERE task_type = 'IMPORT_CHAPTER' AND (started_at >= $1 OR (status IN ('IMPORTING', 'COMPLETED') AND updated_at >= $1))
  `, [windowStartIso]);
  ...
  const claimDbStats = diagReport?.jobProfile?.stages?.claimDb || { p50: 0, p95: 0 };
  ...
  const results = {
    ...
    claimMetrics: {
      ...
      specificClaimP50: Math.round(claimDbStats.p50 * 10) / 10,
      specificClaimP95: Math.round(claimDbStats.p95 * 10) / 10
    },
    mutexStats: {
  ...
  fs.writeFileSync('./validation_5m_fase5_result.json', JSON.stringify(results, null, 2));
  console.log('\n============================================================');
  console.log('FASE 5 - 5-MINUTE VALIDATION RESULT');
  console.log('============================================================');
