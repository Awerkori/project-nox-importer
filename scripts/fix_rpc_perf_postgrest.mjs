...
const sql = `
CREATE INDEX IF NOT EXISTS idx_importer_queue_workid ON public.importer_queue ((payload->>'workId')) WHERE status in ('QUEUED', 'RETRY') AND task_type = 'IMPORT_CHAPTER';
    ...
    FROM (
      ...
      ( 
        ...
        WHERE q_cand.status in ('QUEUED', 'RETRY') 
          ...
          AND not exists (SELECT 1 FROM public.importer_sources s WHERE s.id = q_cand.source AND (s.enabled = false or s.status in ('PAUSED', 'DISABLED', 'UPSTREAM_BLOCKED') or (s.cooldown_until is not null and s.cooldown_until > now())))
        ORDER BY q_cand.chapter_sort_key ASC LIMIT 50
      )
    ) cand_batch
    ORDER BY
