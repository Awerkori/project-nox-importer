...
async function run() {
  const report = JSON.parse(readFileSync('duplicates-report-all.json', 'utf8'));
  
  const targetGroups = report.groups.filter(g => {
    return g.confidence === 'HIGH_CONFIDENCE' || 
           g.shared_title.includes('magic-emperor') || 
           g.shared_title.includes('martial-peak') || 
           g.shared_title.includes('the-saint-levels-up') ||
           g.shared_title.includes('imperador-magico') ||
           g.shared_title.includes('pico-marcial')
  });
  
  console.log(`Starting safe merge for ${targetGroups.length} groups.`);
